...
5. Upgrade shared cluster components (ibm-licensing, ibm-scheduler)
...
9. Apply hotfixes (CCS, Pipelines, DataStage)
...
Before you remove any hot fixes (image_digests) take a backup of the CR yaml
```
oc get <cr kind> <cr name> -n <operand namespace> -o yaml > <cr name>-backup-$(date +%Y%m%d-%H%M%S).yaml
```

To remove the hot fex, remove the entire image_digests section using a patch command
```
...
## 9. Apply patches (DataStage Hotfixes):
